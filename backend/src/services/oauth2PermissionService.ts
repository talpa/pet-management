...
export class OAuth2PermissionService {
  static async assignPermissionsToUser(user: User): Promise<void> {
    try {
      ...
      if (user.role !== mapping.role) {
        user.role = mapping.role as 'admin' | 'user';
        await user.save();
  ...
  private static async assignDirectPermissions(user: User, permissionNames: string[]): Promise<void> {
    try {
      ...
      const existingPermissionNames = existingUserPermissions.map((up: any) => up.permission.name);
      ...
      await UserPermission.bulkCreate(userPermissionsToCreate);
  ...
  static async getUserPermissions(userId: number): Promise<any[]> {
    try {
      ...
      return directPermissions.map((permission: any) => ({
        id: permission.id,
  ...
  static async removeAllUserPermissions(userId: number): Promise<void> {
    try {
